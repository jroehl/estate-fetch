...
const processEstate = (estate, options = {}) => {
  const {
    ...
    excludedKeys = ['previewimage', 'characteristics', 'owner', 'creator', 'modifier', 'portalpublishing', 'children', 'origin'],
  } = options;
  ...
  const availableFeatures = details.filter(({ name }) => features.includes(name));
  ...
  return {
    ...
    rooms,
    availableFeatures,
    commission: commissions.prospectcommission
      ? {
